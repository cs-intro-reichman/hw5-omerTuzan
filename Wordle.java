...
public class Wordle {
    ...
    public static String[] readDictionary(String filename) {
        In newIn = new In(filename);
        String[] fileData = newIn.readAllLines();
        return fileData;
    ...
    public static void computeFeedback(String secret, String guess, char[] resultRow) {
		for (int i = 0; i < guess.length(); i++) {
            if (containsChar(secret, guess.charAt(i))) {
                if (secret.charAt(i) == guess.charAt(i)) {resultRow[i] = 'G'; continue;}
                resultRow[i] = 'Y';
                continue;                
            }
            resultRow[i] = '_';
    ...
    public static boolean isAllGreen(char[] resultRow) {
		for (char result : resultRow) {
            if (result != 'G') {return false;}
        }
        return true;
    }
    ...
    public static void main(String[] args) {
        ...
        if (!won) {
            System.out.println(String.format("Sorry, you did not guess the word.%n" + //
